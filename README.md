...
This starter is derived from [`hngprojects/react-native-starter`](https://github.com/hngprojects/react-native-starter)
(original author: Jeffery Orazulike). It was re-rooted here because **upstream is
compromised**.

### What was removed

Upstream `babel.config.js` carried an obfuscated implant appended after ~500
characters of whitespace padding, so it sat off-screen in an editor while the
first 15 lines looked like a normal Expo config. The payload used
**EtherHiding**: it read attacker-controlled bytecode from Ethereum contract
storage via public RPC endpoints, decompressed it, and executed it — a design
that keeps working even after the C2 domains are taken down.

`babel.config.js` executes on **every build**, so the implant would fire on
`pnpm start`, `pnpm test`, and in CI.

The clean 298-byte config was restored from commit `8bb053e8`, which is the last
commit where upstream legitimately removed the payload. No other file in the tree
was affected — `package.json` had no install hooks and the lockfile resolved
entirely to the default npm registry.

### Do not re-pull from upstream

Upstream's history shows the implant being removed once and then **reintroduced
twice**, in commits both titled *"security: merge automated supply-chain fix for
main"*. The reinfection is disguised as a security fix, so the commit subject is
not a safe signal.
...
(`child_process`, `eval`, `zlib`, raw sockets), and blockchain-C2 RPC patterns.

The scanner is deliberately vendored rather than downloaded at scan time —
fetching it over the network would reintroduce exactly the class of dependency
it exists to catch.
